...
#This code is taken from https://gist.github.com/sixtenbe/1178136,
# which was provided without a license.
# It derives from matlab code that explicitly is stated to be publicly
...
def _datacheck_peakdetect(x_axis, y_axis):
    ...
    if len(y_axis) != len(x_axis):
        raise ValueError('Input vectors y_axis and x_axis '
...
def _smooth(x, window_len=11, window='hanning'):
    ...
    if not window in ['flat', 'hanning', 'hamming', 'bartlett', 'blackman']:
        raise ValueError("Window is not one of '{0}', "
                         "'{1}', '{2}', '{3}', '{4}'"
                         "".format(*('flat', 'hanning', 'hamming',
                                     'bartlett', 'blackman')))
...
def zero_crossings(y_axis, window = 11):
    ...
    if diff.std() / diff.mean() > 0.2:
        ...
        raise ValueError("False zero-crossings found, indicates problem {0} "
                         "or {1}".format("with smoothing window",
                                         "problem with offset"))
...
#        raise ValueError("smoothing window too small, "
#                         "false zero-crossing found")
#    
#    #return frequency
